...
before packages are loaded."
  ...
  (with-eval-after-load 'org
    ...
    (add-hook 'org-mode-hook #'text-stuff)
    (add-hook 'org-mode-hook #'org-indent-mode))

  (with-eval-after-load 'markdown-mode
    (add-hook 'markdown-mode-hook #'text-stuff)
  ...
  (when (spacemacs/system-is-mac)
    ...
    ;; These work with the Darwin port but not the Mac port
    (global-set-key (kbd-mac-command "M-h") 'ns-do-hide-others) ; Darwin port only
    (global-set-key (kbd-mac-command "`") 'other-window)        ; Darwin port only
    )
...
(custom-set-faces
 ...
 '(variable-pitch ((t (:height 1.2 :family "PT Sans")))))
)
